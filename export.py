...
def resnet_fpn_model_fn(features, labels, mode, params):
    ...
    if mode != tf.estimator.ModeKeys.PREDICT:
        multilevel_anchors = [RPNAnchors(all_anchors_fpn[i], features['anchor_labels_lvl{}'.format(i + 2)],
                                         features['anchor_boxes_lvl{}'.format(i + 2)]) for i in
                              range(len(all_anchors_fpn))]
    else:
    ...
    multilevel_pred_boxes = [anchor.decode_logits(logits) for anchor, logits in
                             zip(multilevel_anchors, multilevel_box_logits)]
    proposal_boxes, proposal_scores = generate_fpn_proposals(multilevel_pred_boxes, multilevel_label_logits,
                                                             image_shape2d, is_train)
    ...
    if not _C.FPN.CASCADE:
        ...
        fastrcnn_head = CascadeRCNNHead(proposals, roi_func, fastrcnn_head_func,
                                        (gt_boxes, gt_labels), image_shape2d, num_classes,
    ...
    if mode != tf.estimator.ModeKeys.PREDICT:
        ...
        if is_train:
            learning_rate = tf.train.piecewise_constant(global_step, lr_schedule,
                                                        values=[tf.convert_to_tensor(0.01 * 0.33 * 0.375,
                                                                                     tf.float32)] + [
                                                                   learning_rate * (0.1 ** i) for i in
                                                                   range(len(lr_schedule))])
