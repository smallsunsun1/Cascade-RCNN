...
class SequentialBatchFFTGPU : public OpKernel {
 public:
  explicit SequentialBatchFFTGPU(OpKernelConstruction *context) : OpKernel(context) {
      OP_REQUIRES_OK(context, context->GetAttr("compute_size", &compute_size_));
      OP_REQUIRES(context, compute_size_ >= 1,
                  errors::InvalidArgument("Need compute_size >= 1, got ", compute_size_));
  }
  void Compute(OpKernelContext *context) override {
      auto stream = context->op_device_context()->stream();
      OP_REQUIRES(context, stream, errors::Internal("No GPU stream avaliable."));
      // Grab the input tensor
      const Tensor &input_tensor = context->input(0);
      // input should have 2 dimensions.
      OP_REQUIRES(context, input_tensor.dims() == 2,
                  errors::InvalidArgument("Inout must be 2-dimensional"));
      // input size
      ...
      int actual_compute_size = (compute_size_ > batch_size ? batch_size : compute_size_);
      OP_REQUIRES(context, SequentialFFTCUDAKernel(input_data, batch_size, dim,
                                                   actual_compute_size, Forward, output_data),
                  errors::Internal("cuFFT kernel excution failed: input.shape=",
                                   shape.DebugString()));
  }
 private:
  int compute_size_;
...
REGISTER_KERNEL_BUILDER(Name("SequentialBatchFFT").Device(DEVICE_GPU).TypeConstraint<complex64>("T"),
                        SequentialBatchFFTGPU<complex64, true>);
REGISTER_KERNEL_BUILDER(Name("SequentialBatchFFT").Device(DEVICE_GPU).TypeConstraint<complex64>("T"),
                        SequentialBatchFFTGPU<complex64, false>);
REGISTER_KERNEL_BUILDER(Name("SequentialBatchFFT").Device(DEVICE_GPU).TypeConstraint<complex128>("T"),
